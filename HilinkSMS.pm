...
sub api_post_named {
    ...
    $xml_body = '<?xml version="1.0" encoding="UTF-8"?><request></request>' unless defined $xml_body && length $xml_body;
    return hi_post_xml($POST_API{$name}, $xml_body, "$MODEM/html/index.html");
}
...
sub send_sms {
    ...
    my $reserved = $opts{reserved} // 0;
    $reserved = 0 unless $reserved =~ /^-?\d+$/;
    my $sca = $opts{sca} // '';
...
sub process_action {
    ...
    if ($action eq 'send') {
        ...
        return $ok ? job_to_xml(id => $id_or_err, status => 'queued') : job_to_xml(error => $id_or_err);
    }
    elsif ($action eq 'job') {
        return get_job_xml($q->param('id') || '');
    }
        ...
        return probe_sms_list();
    }
    elsif ($action eq 'api-list') {
        return api_list_xml();
    }
    elsif ($action eq 'get') {
        return api_get_named($q->param('name') || '');
    }
    elsif ($action eq 'post') {
        return api_post_named($q->param('name') || '', $q->param('xml') || '');
    }
    elsif ($action eq 'status') {
        return api_get('api/monitoring/status');
    }
    elsif ($action eq 'info') {
        return api_get('api/device/information');
    }
    elsif ($action eq 'signal') {
        return api_get('api/device/signal');
    }
    elsif ($action eq 'sms-count') {
        return api_get('api/sms/sms-count');
    }
    elsif ($action eq 'sms-config') {
        return api_get('api/sms/config');
    }
    elsif ($action eq 'change-password') {
        my $old = $q->param('old') || '';
        my $new = $q->param('new') || '';
        if (length $new < 4) { return "ERROR: password too short"; }
        ...
        if ($? == 0) {
            ...
            if ($new_hash) {
                ...
                if ($out) { print {$out} "admin:$new_hash\n"; close $out; log_msg('INFO', 'password changed'); return "OK"; }
                else { return "ERROR: write failed"; }
